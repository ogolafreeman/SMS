...
if (isset($_SESSION['username']) && isset($_SESSION['teacher_role'])) {
	...
	$sql8 = "SELECT staff_id FROM staff_tbl WHERE nic='" . $_SESSION['username'] . "'";
	$result8 = mysqli_query($con, $sql8);
	$row5 = mysqli_fetch_assoc($result8);
	$staff_id = $row5['staff_id'];

	$sql1 = "SELECT * FROM student_tbl WHERE std_id IN (
			 SELECT sct.std_id from grade_class_tbl gct inner join student_class_tbl sct ON 
			 (gct.grade_class_id = sct.grade_class_id) where gct.staff_id='$staff_id') AND admission_no='$admission_no'";
		...
		if (mysqli_num_rows($result2) == 1) {
			...
			if (mysqli_num_rows($result3) > 0) {
				while ($row3 = mysqli_fetch_assoc($result3)) {
					...
					$sql4 = "SELECT * FROM al_marks_tbl WHERE term='$term' AND sub_id='$sub_id' AND grade_id='$algrade' AND std_id='$std_id' AND year='$year' AND NOT marks=''";
